...
export type GoogleMessageRole = "user" | "model";
...
export interface GoogleGenerateContentSuccess {
  ok: true;
  content: string | readonly GoogleContentPart[];
  usage?: GoogleUsage;
}

export interface GoogleGenerateContentFailure {
  ok: false;
  statusCode?: number;
...
export type GoogleGenerateContentResult =
  | GoogleGenerateContentSuccess
  | GoogleGenerateContentFailure;
...
export interface GoogleDiscoveryAdapterOptions {
  apiKey: string;
  model: string;
  ...
  runGenerateContent: GoogleGenerateContentClient;
...
export interface GoogleDiscoveryAdapterUsage {
  promptTokens: number;
  completionTokens: number;
  totalTokens: number;
}

export interface GoogleDiscoveryAdapterResult {
  discovery: DiscoveryResult;
  model: string;
  turns: number;
  turnDurationsMs: number[];
  usage: GoogleDiscoveryAdapterUsage;
...
function classifyGenerateContentFailure(
  error: GoogleGenerateContentFailure,
): LlmFailureKind {
...
function incrementUsage(
  usage: GoogleDiscoveryAdapterUsage,
  delta: GoogleUsage | undefined,
): void {
  if (!delta) {
    return;
  }
  const promptTokens = readTokenCount(delta.promptTokenCount);
  const completionTokens = readTokenCount(delta.candidatesTokenCount);
  const totalTokens =
    delta.totalTokenCount !== undefined
      ? readTokenCount(delta.totalTokenCount)
...
function extractMessageText(content: GoogleGenerateContentSuccess["content"]): string {
...
async function resolveFallbackFromState(
  options: GoogleDiscoveryAdapterOptions,
  ...
  shouldRetry: boolean;
  delayMs: number;
  result?: GoogleDiscoveryAdapterResult;
}> {
...
export async function runGoogleDiscoveryAdapter(
  options: GoogleDiscoveryAdapterOptions,
): Promise<GoogleDiscoveryAdapterResult> {
  const sleep = options.sleep ?? defaultSleep;
  const messages: GoogleMessage[] = [
    { role: "user", text: options.initialUserPrompt },
  ];
  const usage: GoogleDiscoveryAdapterUsage = {
    promptTokens: 0,
    completionTokens: 0,
    totalTokens: 0,
  };
  ...
  while (true) {
    ...
    if (gate.shouldRequestFinal) {
      messages.push({ role: "user", text: LAST_TURN_CTX_FINAL_MESSAGE });
    }

    const turn = turnManager.startTurn();
    const response = await options.runGenerateContent({
    ...
    if (!response.ok) {
      const failure = classifyGenerateContentFailure(response);
    ...
    messages.push({ role: "model", text: assistantContent });
    ...
    if (calls.length > 0) {
      ...
      messages.push({ role: "user", text: dispatch.envelopes.join("\n\n") });
    }
    ...
    if (finalBlocks.payloads.length > 0 || finalBlocks.errors.length > 0) {
      if (finalBlocks.payloads.length > 0) {
        ...
        if (validation.action === "retry") {
          ...
          messages.push({
            role: "user",
            text: describeValidationIssues(combinedIssues),
          });
          continue;
        }
      ...
      if (fallback.shouldRetry) {
        ...
        messages.push({
          role: "user",
          text: describeValidationIssues(finalBlocks.errors),
        });
        continue;
      }
      if (!fallback.result) {
    ...
    if (calls.length === 0) {
      ...
      if (fallback.shouldRetry) {
        ...
        messages.push({
          role: "user",
          text: "No ctx_final block found. Output a valid ctx_final block now.",
        });
        continue;
      }
      if (!fallback.result) {
