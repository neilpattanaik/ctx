...
import {
  runGeminiCliDiscoveryAdapter,
  type GeminiCliInvocationRequest,
} from "../../src/discovery/gemini-cli-adapter";
...
describe("runGeminiCliDiscoveryAdapter", () => {
  test("returns validated discovery when output contains ctx_final", async () => {
    const requests: GeminiCliInvocationRequest[] = [];

    const result = await runGeminiCliDiscoveryAdapter({
      command: "gemini",
    ...
    expect(requests[0]?.command).toBe("gemini");
  });
