...
import {
  runGoogleDiscoveryAdapter,
  type GoogleGenerateContentResult,
} from "../../src/discovery/google-adapter";
...
describe("runGoogleDiscoveryAdapter", () => {
  test("returns validated discovery when assistant emits ctx_final", async () => {
    const requests: Array<{ messages: string[]; systemInstruction: string }> = [];

    const result = await runGoogleDiscoveryAdapter({
      apiKey: "AIzaFakeKeyForTestsOnly123456789012345",
      ...
      runGenerateContent: async (request) => {
        requests.push({
    ...
    expect(requests[0]?.systemInstruction).toBe("system prompt");
  });
  ...
  test("dispatches tool calls and continues until ctx_final is emitted", async () => {
    const responses: GoogleGenerateContentResult[] = [
      {
        ok: true,
        content: [
          {
